            ...
            const vec<vec<T>>& all_closures)
    {
        ...
        for(const auto& io : iorb){
            if(io.second>estart[io.first]){
                ...
                T emm = all_closures[e_inst[pe].c][e_inst[pe].m];
            ...
            const vec<vec<T>>& all_closures)
    {
        ...
        for(const auto& io : iorb){
            if(io.second < estart[io.first+1]-1){
                // find orbit of io.second+1
                int64_t pe = io.second+1;
                T epp = all_closures[e_inst[pe].c][e_inst[pe].m];
                int cID = e_inst[pe].eoID;
    ...
    int RecursiveWalk(vec<T>& unipath, const vec<etuple<EL>>& e_inst,
        ...
        while(j<all_closures[e_inst[nv].c].size()-e_inst[nv].m){
            ...
            if(e_inst[estart[e_inst[nv].c]+e_inst[nv].m+j].type>1){
                // get eorbit
                T ed = all_closures[e_inst[nv].c][e_inst[nv].m+j];
        ...
        const Bool verbose, String dir, Bool Canon)
{
    ...
    int64_t nummatch = 0, stopped = 0, dots = 0;
    int64_t l1, l2;
    ...
    for(T e = 0; e < hbE; e++){
        ...
        for(const auto& orb : orbits){
            BuildSuperGraph::IsLeftMost<T,EL>(e_inst,estart,orb,all_closures);
        } 
    }
    #pragma omp parallel for schedule( dynamic, 1000 )
    for(T e = 0; e < hbE; e++){
        ...
        //for each orbit
        for(const auto& orb : orbits){
            BuildSuperGraph::IsRightMost<T,EL>(e_inst,estart,orb,all_closures);
        } 
    }
    ...
    for(int i = 0; i< repL.isize(); i++){
        BuildSuperGraph::SmartJoinR2L<T,EL>(v_inst, e_inst, estart, repL[i], map_ei, all_closures);
        if(verbose){
            #pragma omp critical
    ...
    for(int iv = 0; iv < repL.isize(); iv++){
        permRepR[iv] = BinPosition(repR,BuildSuperGraph::RecursiveWalk<T,EL>(
                    unipaths[iv],e_inst,estart,v_inst[repL[iv]].v,
                    all_closures, map_ei, GET_META, metadata[iv]));
...
template void NucleateGraph(vec<vec<om<uint16_t>>>& omatch, int hbE, 
        vec<vec<int>>& all_closures, vec<int64_t>& cinv, 
        vec<vec<int>>& ci, digraphE<vec<int>>& D, vec<int>& dinv, 
        const Bool GET_META,
        vec<vec<quad<int,uint16_t,int,uint16_t>>>& closure_supp,
        const Bool verbose, String dir, Bool Canon);

// for purely base sequence
template void NucleateGraph(vec<vec<om<uint16_t>>>& omatch, unsigned char hbE, 
        vec<vec<unsigned char>>& all_closures, vec<int64_t>& cinv, 
        vec<vec<int>>& ci, digraphE<vec<unsigned char>>& D, vec<int>& dinv,
        const Bool GET_META,
        vec<vec<quad<int,uint16_t,int,uint16_t>>>& closure_supp,
        const Bool verbose, String dir, Bool Canon);
