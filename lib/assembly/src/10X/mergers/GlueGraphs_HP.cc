...
namespace ExtractGraphs_HP{
    ...
    void IsLeftMost(vec<etuple<EL>>& e_inst, const vec<int64_t>& estart, 
        ...
        T emm = All_Closures::elem(D,Q.back().first,Q.back().second);
        Q = eorbits[emm][e_inst[start].eoID];
    ...
    void IsRightMost(vec<etuple<EL>>& e_inst, const vec<int64_t>& estart, 
        ...
        if(pr>-1)
            ...
            Q.push(rightds[0],0);
        int64_t start = estart[Q.back().first]+Q.back().second;
        T epp = All_Closures::elem(D,Q.back().first,Q.back().second);
        Q = eorbits[epp][e_inst[start].eoID];
            ...
            const vec<vec<vec<pair<int,EL>>>>& eorbits)
    {
        ...
        const vec<pair<int,EL>>& iorb = eorbits[ed][e_inst[ne].eoID];
        ...
        for(const auto& io : iorb){
            if(io.second>0){
                ...
                int64_t pe = estart[io.first]+io.second-1;
            ...
            const vec<vec<vec<pair<int,EL>>>>& eorbits)
    {
        ...
        for(const auto& io : iorb){
            if(io.second < All_Closures::isize(estart,io.first)-1){
                // find orbit of io.second+1
                int64_t pe = estart[io.first]+io.second+1;
    ...
    int Walk(vec<T>& unipath, const vec<etuple<EL>>& e_inst,
        ...
        int64_t idx;
        T el = All_Closures::elem(D,e_inst[nv].c,e_inst[nv].m+j-1);
        vec<pair<int,EL>> Q = eorbits[el][e_inst[estart[e_inst[nv].c]+e_inst[nv].m+j-1].eoID];
    ...
    int RecursiveWalk(vec<T>& unipath, const vec<etuple<EL>>& e_inst,
        ...
        while(k<0){
            k = Walk<T,EL>(unipath, e_inst, estart, nv, 
                    D, map_ei, to_rights, GET_META, metadata, eorbits);
        }
        return k;
    }
}
...
template <class T, class EL>
void GlueGraphs_HP(digraphE<vec<T>>& D, vec<int>& dinv, vec<nptuple>& Matches, 
        ...
        const Bool verbose, String dir)
{
    ...
    int nummerges = 0, stopped = 0, dots = 0;
    for(int64_t i = 0; i < (int64_t) Matches.size(); i++){
        ...
        if(verbose)
            MakeDots(stopped,dots,Matches.isize());
    }
    ...
    if (verbose) PRINTDEETS("precompute eorbits and canonicalize classIds");

    if(dir!=""){
        BinaryReader::readFile(dir+"/a.ci",&ci);
    }

    vec<vec<vec<pair<int,EL>>>> eorbits(nE);
    ...
    for(T e = 0; e < nE; e++){
        ...
        // for each orbit
        for(unsigned int k = 0; k < orbits.isize(); k++)
            for(auto q: orbits[k])
    ...
    for(T e = 0; e < nE; e++){
        ...
        for(const auto& orb : orbits)
            ExtractGraphs_HP::IsLeftMost<T,EL>(e_inst,estart,orb,D,to_lefts,to_rights,hbE,eorbits);
    }
    #pragma omp parallel for schedule( dynamic, 1 )
    for(T e = 0; e < nE; e++){
        // get partitions/orbits of (c,m) wrt e
        vec<vec<pair<int,EL>>>& orbits = eorbits[e];
        for(const auto& orb : orbits)
            ExtractGraphs_HP::IsRightMost<T,EL>(e_inst,estart,orb,D,to_lefts,to_rights,hbE,eorbits);
    }
    ...
    for(int i = 0; i< repL.isize(); i++){
        ExtractGraphs_HP::SmartJoinR2L<T,EL>(v_inst,e_inst,estart,repL[i],D,map_ei,to_lefts,eorbits);
        if(verbose){
            #pragma omp critical
    ...
    for(int iv = 0; iv < repL.isize(); iv++){
        permRepR[iv] = BinPosition(repR,ExtractGraphs_HP::RecursiveWalk<T,EL>(
                    unipaths[iv], e_inst,estart,v_inst[repL[iv]].v, 
                    D, map_ei, to_rights, GET_META, metadata[iv],eorbits));
    ...
    for(int p = 0; p < unipaths.isize(); p++){
        ...
        T ed = All_Closures::elem(D,e_inst[ne].c,e_inst[ne].m);
        vec<pair<int,EL>>& Q = eorbits[ed][e_inst[ne].eoID];
        vec<int64_t> RCQ;
        for(const auto& q: Q)
