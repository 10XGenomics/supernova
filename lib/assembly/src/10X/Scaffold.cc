...
// PutativeJoins - return vec<> of pairs of line (.first) followed by a successor (.second) - business rules may need tweaking
//

#if 0
vec<pair<int,int>> PutativeJoins( const HyperBasevectorX& hb, const vec<int>& inv,
     const digraphE<vec<int>>& D, const vec<int>& dinv, const vec<int64_t>& bci,
...
// Lhood0 - set up and call Lhood1
//
void Lhood0( const int L, const HyperBasevectorX& hb, const vec<int>& inv,
     const digraphE<vec<int>>& D, const vec<int>& dinv, const vec<int64_t>& bci,
     ReadPathVec& dpaths, const vec<Bool>& dup, const ReadPathVecX& pathsx, 
     const vec< triple<int,int,int>>& qept, const VecIntVec& ebcx
     )
...
// Lhood1 - implementation of the lhood neighborhood as seen in Probe
vec<quad<int,double,int,int>> Lhood1( const int L, 
          vec<vec<vec<vec<int>>>> dlines,
          vec<int> const& llens,
          vec<int> const& linv,
          vec<double> const& cov,
          vec< vec< pair<int,int>>> const& lhood,
          MasterVec<SerfVec<pair<int,int>>> const& lbpx,
          int const MIN_LEN = 2000,                          // min length of line to consider 
          bool skip_inv = true
          )
{
     // return value below to be forwarded
     vec<quad<int,double,int,int>> nhood;    // ( left/right, -score, L2, b )

     int S1 = 40; // low score to print
          
     // Define constants.
     const int S2 = 1000000000; // high score to print
     ...
     {    
          ...
          if ( skip_inv && ( L2 == L || L2 == linv[L] ) ) continue;
     ...
     {    int L2 = LH[i].second;
          ...
          scores.push_back( ScoreOrder( { L2, L }, lbpx, llens, M ) );
          scores.push_back( ScoreOrder( { linv[L2], L }, lbpx, llens, M ) );
          scores.push_back( ScoreOrder( { L, L2 }, lbpx, llens, M ) );
          scores.push_back( ScoreOrder( { L, linv[L2] }, lbpx, llens, M ) );
          vec<int> ids( 4, vec<int>::IDENTITY );
          SortSync( scores, ids );
          double win = scores[1] - scores[0];
          if ( win < S1 || win > S2 ) continue;
          #pragma omp critical
          {    nhood.push( ( ids[0]==0||ids[0]==1 ? -1 : +1 ),
                      -win, ( ids[0]==0||ids[0]==2 ? L2 : linv[L2] ),
                      LH[i].first );    }    }
     Sort(nhood);
     ...
     const Bool single )
{
     ...
     { // SCOPING to kill lpaths and lpaths_index
...
// Lower memory version of Scaffold function above
// Just avoids creating lpaths and lpaths_index
     ...
     const Bool single )
{
     ...
     if (verbose) cout << Date( ) << ": indexing dpaths" << endl;
     ...
     for ( int l1 = 0; l1 < lines.isize( ); l1++ )
     {
          ...
          vec<int64_t> ids;
          for ( auto & block : lines[l1] ) {
          for ( auto & path  : block ) {
          for ( auto & d : path ) {
          for ( int i = 0; i < dpaths_index.Count( d ); i++ )
               ids.push_back( dpaths_index.Val( d, i ) );
          }    }    }
          UniqueSort( ids );
          ...
          {    
               ...
               const ReadPath& p1 = dpaths[id1];
               const int loff1 = p1.getOffset( ) + tol[p1[0]].second;
               int dist1 = lens[l1] - loff1;

               // Get distance of second read start from right end of line l2.

               const int64_t id2 = ( id1 % 2 == 0 ? id1+1 : id1-1 );
               const ReadPath& p2 = dpaths[id2];
               if ( p2.size( ) == 0 ) continue;
               int l2 = tol[p2[0]].first;
               const int loff2 = p2.getOffset( ) + tol[p2[0]].second;
               int dist2 = lens[l2] - loff2;
