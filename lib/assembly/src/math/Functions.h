...
template <class V> inline Float WeightedMean( V const& v )
{    Float sum1 = 0, sum2 = 0;
     for ( Float val : v )
     {    sum1 += val;
          sum2 += val*val;    }
...
/// "N50" of the original list.  Input list does not have to be sorted.  If v is
/// empty, return 0.  N50PL is just like N50 but is piecewise-linear at boundaries,
/// and hence less sensitive to minute changes.  
/// NPL is like N50; NPL(x,0.5) = N50PL(x).
///
// ==========================================================================
