                                               ...
                                               float * score)
{
  ...
  return NormalDistribution(mu_sig2 * sig2, sqrt(sig2));
}
                           ...
                           const float n_sigma)
{
  ...
  for (size_t i = 1; i != n; i++) {
    ...
    if (overlap) {
      ...
      clusters.push_back(vec<NormalDistribution>(1, myds[i]));
      i_cluster++;
    }
  }
...
{    if ( v.empty( ) ) return 0;
     ...
     {    ForceAssert( v[i] > 0 );
          sum += v[i];    }
     if ( is_sorted( v.rbegin(), v.rend() ) ) // reverse sorted case
     {    for ( int i = v.size()-1; i >= 0; i-- ) 
          {    half += v[i];
               if ( 2 * half == sum && i > 0 ) 
                    return (v[i] + v[i-1])/2;
               if ( 2 * half >= sum ) return v[i];    }
...
template<class T> T N50PL( const vec<T>& v )
{    if ( v.empty( ) ) return 0;
     longlong sum = 0, half = 0;
     for ( int i = 0; i < (int)v.size( ); i++ ) 
     {    ForceAssert( v[i] > 0 );
     ...
     {    sorted_v = v;
          use_given_v = false;
          Sort(sorted_v);    }
     const vec<T>& x = ( use_given_v ) ? v : sorted_v;
     for ( int i = 0; i < (int) x.size( ); i++ ) 
     {    longlong halfp = half;
          half += x[i];
          if ( 2*halfp <= sum && sum <= 2*half )
          {    int64_t low = ( i > 0 ? x[i-1] : 0 ), high = x[i];
               double frac = double( sum - 2*halfp ) / double( 2*half - 2*halfp );
               return ((1-frac) * low) + (frac * high);    }    }
     return 0;    } // never executed
template int N50PL( const vec<int>& v );
template unsigned int N50PL( const vec<unsigned int>& v );
template longlong N50PL( const vec<longlong>& v );
template unsigned long N50PL( const vec<unsigned long>& v );

template<class T> T NPL( const vec<T>& v, const double F )
{    if ( v.empty( ) ) return 0;
     longlong sum = 0, half = 0;
     for ( int i = 0; i < (int)v.size( ); i++ ) 
     {    ForceAssert( v[i] > 0 );
     ...
     {    longlong halfp = half;
          half += x[i];
          if ( halfp <= sum*(1-F) && (1-F)*sum <= half )
          {    int64_t low = ( i > 0 ? x[i-1] : 0 ), high = x[i];
               double frac = double( (1-F)*sum - halfp ) / double( half - halfp );
               return ((1-frac) * low) + (frac * high);    }    }
     return 0;    } // never executed
template int NPL( const vec<int>& v, const double F );
template unsigned int NPL( const vec<unsigned int>& v, const double F );
template longlong NPL( const vec<longlong>& v, const double F );
template unsigned long NPL( const vec<unsigned long>& v, const double F );
