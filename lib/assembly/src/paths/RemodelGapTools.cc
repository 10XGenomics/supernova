...
template void MakeKmerLookup0( const vecbasevector& tigs,
     vec< triple< kmer<8>,int,int> >& kmers_plus );
     ...
     Bool& gap_predicted, int& gap, double& dev )
{
     ...
     double ndev = 3.0;
     double p = NormalDistributionIntegral( ndev, 0.0, 1.0 );
     ...
     vec< vec<double> >& X )
{
     ...
     {    for ( int l = 0; l < nlibs; l++ )
          ...
          if ( pass == 2 ) {
              cout << "Number of links after pass 2 : " << endl;
              for ( int i = 0; i < nlibs; i++ ) {
                  String flag = ( lfail[i] ? "True" : "False" );
                  cout << "Lib " << i << " " << DISTS[i].size() << " lfail= " << flag <<  endl;
                  if ( DISTS[i].empty() && ! lfail[i] ) {
                      cout << "    Disable lib " << i << " since no links are found in pass 2 " << endl;
                      lfail[i] = True;
                  } 
              }
          }
