...
class align {
 ...
 public:
  ...
  void PrintMutations( const basevector& rd1, const basevector& rd2, ostream& log, const bool zero_based=true) const;
  ...
  int MaxPerf( const basevector& rd1, const basevector& rd2 ) const
  {    int perf = 0;
       vec<ho_interval> perfs;
       PerfectIntervals1( rd1, rd2, perfs );
       for ( auto h : perfs ) perf = Max( perf, h.Length( ) );
       return perf;    }
  ...
  void CreateFromMutmersAndSW( int k, shortvector<mutmer>& m, 
       ...
       double end_stretch, int& errors_found, bool affine_penalties );
