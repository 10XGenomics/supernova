...
*/
pub fn best_middle_base(qv: u8, kmer: Kmer, kmer_counts: &FxHashMap<Kmer, u32>) -> u8 {
    let counts = kmer_counts.get(&kmer.min_rc()).map_or(0, |x| *x);
    if counts >= 3 {
        return kmer.get(K/2)
    }
    ...
    for i in 0 .. 4 {
        let cand_kmer = kmer.set(K/2, i).min_rc();
...
/// Feed in seq in sequencer order -- starting with good quality reads is better!
pub fn correct_seq(read: &Vec<u8>, qvs: &Vec<u8>, kmer_counts: &FxHashMap<Kmer, u32>) -> Vec<u8> {
    // find an edit of seq that makes it use only 'good' Kmers
