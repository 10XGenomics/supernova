...
impl<'a, T: 'static + Copy + Send> Shard<'a, T> {
    ...
    pub fn get_producer_handle(&self) -> ShardProducer<T>
    {
        ShardProducer::new(&self)
    }
...
impl<'a, T: Copy + Send> ShardProducer<T> {
    fn new(shard: &Shard<'a, T>) -> ShardProducer<T> {
...
mod pod_tests {
    ...
    fn pod_round_trip() {
        ...
        // Write and shut down
        {
            let mut w = super::PodWriter::new(tmp.path());
            w.write(&write_vec);
        }

        let mut reader = super::PodReader::new(tmp.path());
