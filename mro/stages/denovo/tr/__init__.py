...
def main(args, outs):
    ...
    tr_command = ['TR','DIR='+input_dir, 'MAX_MEM_GB='+str(args.__mem_gb), 'NUM_THREADS='+str(args.__threads)]

    if args.known_sample_id is not None:
        tr_command.append( "SAMPLE={}".format(args.known_sample_id) )
    if args.addin is not None and "TR" in args.addin:
        tr_command.extend(args.addin["TR"].split())


    ## write alerts
    alerts.write_stage_alerts("tr", path=args.parent_dir)

    alarm_bell = alerts.SupernovaAlarms(base_dir=args.parent_dir)
    try:
        subprocess.check_call( tr_command )
